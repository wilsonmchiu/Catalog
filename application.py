...
def newCategory():
    ...
    if request.method == 'POST':
        ...
        if category_exists is True:
            ...
            flash_string = ("%s is already in use. \
                Provide a different name" % newCategory.name)
            flash(flash_string)
...
def newCategoryItem(category_id):
    ...
    if login_session['user_id'] != category.user_id:
        ...
        'You are not authorized to add category items to \
        this category. Please create your own category in order to \
        add items.');} \
        </script><body onload='myFunction()'>"
        ...
        if category_item_exists is True:
                # Name has been taken, reload same page and flash message
            flash_string = ("%s is already in use. \
                Provide a different name" % newItem.name)
            flash(flash_string)
...
@app.route(
           ...
           <int:category_item_id>/edit', methods=['GET', 'POST'])
...
@app.route('/catalog/<int:category_id>/items/\
           <int:category_item_id>/delete', methods=['GET', 'POST'])
